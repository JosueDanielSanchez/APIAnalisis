...
app.post('/verify-face', upload.single('photo'), async (req, res) => {
  ...
  try {
    ...
    let dbImageURL = user.foto.startsWith('http') ? encodeURI(user.foto) :
      encodeURI(`https://yruggdjexmsxtepcthos.supabase.co/storage/v1/object/public/users/${user.foto}`);
...
// ------------------- Servidor HTTP con timeout -------------------
const PORT = process.env.PORT || 3000;
await loadFaceModels(); // cargar modelos **antes** de levantar el servidor
const server = http.createServer(app);
server.setTimeout(180000); // 180 segundos
